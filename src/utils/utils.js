...
const move = (mousePos, el) => {
    ...
    const tx = lerp(translationVals.tx, map(mousePos.x, 0, winsize.width, -xstart, xstart), 0.3);
    const ty = lerp(translationVals.ty, map(mousePos.y, 0, winsize.height, -ystart, ystart), 0.3);
    const r = lerp(translationVals.r, map(mousePos.x, 0, winsize.width, -randR, randR), 1);
